...
// Proceed with delivery options after address is confirmed
const proceedWithDeliveryOptions = async (phone) => {
...
// Ask for discount code when using saved address
const askForDiscountCode = async (phone) => {
  const discountButtons = [
    {
      type: 'reply',
      reply: {
        id: 'use_discount',
        title: '🎟️ Yes, I have a code'
      }
    },
    {
      type: 'reply',
...
// Proceed with order summary and payment
const proceedWithOrderSummary = async (phone) => {
  const session = sessions[phone];
...
const handleFlowResponse = async (flowResponse, phone) => {
  try {
    ...
    if (session.step === 'awaiting_address_flow') {
      ...
      const deliveryText = session.delivery_type === 'pickup' ? 'Store Pickup' : `Ship to: ${session.address.line}, ${session.address.city}, ${session.address.state} - ${session.address.pincode}`;
...
app.post('/webhook', async (req, res) => {
  ...
  if (type === 'interactive' && msg.interactive?.type === 'button_reply') {
    ...
    switch (buttonId) {
      case 'use_existing_address':
        session.step = 'mobile_for_address';
        await sendMessage(from, '📱 Please enter your 10-digit mobile number (without spaces or country code):');
      ...
      case 'use_saved_address':
        ...
        session.step = 'address_confirmed';
        await askForDiscountCode(from);
        sessions[from] = session;
        break;
        
      case 'edit_saved_address':
        // Send flow to edit address
        if (FLOW_IDS.ADDRESS_FORM) {
          const flowData = {
            name: session.name || '',
            email: session.email || '',
            mobile: session.mobile || '',
            ...
            total_amount: session.total?.toString() || '0',
            currency: 'INR'
          };
          await sendFlowMessage(from, FLOW_IDS.ADDRESS_FORM, flowData);
          session.step = 'awaiting_address_flow';
        } else {
          session.step = 'collect_customer_info';
          await sendMessage(from, '👤 Please enter your *Full Name*:');
        }
      ...
      case 'delivery_ship':
        ...
        await proceedWithOrderSummary(from, session);
        break;
      ...
      case 'use_discount':
        session.step = 'discount_code';
        await sendMessage(from, '🎟️ Please enter your discount code:');
        sessions[from] = session;
        break;
        
      case 'skip_discount':
        await proceedWithDeliveryOptions(from);
        break;
  ...
  if (type === 'order') {
    ...
    // Send address selection message
    ...
    await sendInteractiveMessage(
      from,
      '📦 Address Selection',
      `${summary}\nHow would you like to proceed with the delivery address?`,
      addressButtons
    );
  ...
  // Helper function to proceed with delivery options
  const proceedWithDeliveryOptions = async (phone, session) => {
    const deliveryButtons = [
      {
        type: 'reply',
        reply: {
  ...
  // Helper function to proceed with order summary
  ...
  if (type === 'text') {
    ...
    // Handle mobile number input for existing address
    if (session.step === 'mobile_for_address') {
      const mobileNumber = text.replace(/\D/g, ''); // Remove non-digits
      ...
      const savedAddress = await getAddressFromFirebase(mobileNumber);
      
      if (savedAddress) {
        // Address found, show options
        ...
        session.step = 'address_found';
      } else {
        // No address found, collect new address
        session.mobile = mobileNumber;
        session.step = 'collect_customer_info';
        await sendMessage(from, '📍 No address found for this number.\n\n👤 Please enter your *Full Name*:');
      }
    ...
    if (session.step === 'discount_code') {
      ...
      if (discountRes.valid) {
        ...
        await sendMessage(from, `✅ Discount code "${discountCode}" applied! You saved ₹${discountRes.amount}`);
      } else {
        session.discount_value = 0;
        await sendMessage(from, `❌ Discount code "${discountCode}" is invalid or expired.`);
      }
    ...
    switch (session.step) {
      ...
      case 'collect_address':
        ...
        await sendMessage(from, '🏙️ Please enter your *City*:');
        break;

      case 'collect_city':
        session.address.city = text;
        session.step = 'collect_state';
        await sendMessage(from, '🗺️ Please enter your *State*:');
        break;
...
// POST route for Razorpay webhook (for payment processing)
app.post('/razorpay-webhook', async (req, res) => {
  updateActivity();
  
  console.log('POST /razorpay-webhook accessed with body:', req.body);
  
  // Find session by payment link ID or other identifier
  const phone = Object.keys(sessions).find(key => 
    sessions[key].paymentLinkId === req.body.payment_link_id
  );
